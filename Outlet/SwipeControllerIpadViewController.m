...
@interface SwipeControllerIpadViewController ()

@end

@implementation SwipeControllerIpadViewController
...
SwipeControllerIpadViewController* THIS1 = NULL;

NSString *Decryption_data1 = NULL;
NSString *amountStatus = NULL;

NSString *snd_demo1 = NULL;
char snd_str1[1024] = {0};

extern int DEBUG_MIC;



void checkStatus1(OSStatus status)
{
                                 ...
                                 AudioBufferList *ioData)
{
    ...
    THIS1 = (__bridge SwipeControllerIpadViewController*)inRefCon;
...
- (void) goBack: (id) sender {
    ...
    [self dismissViewControllerAnimated:NO
                             completion:^{
                                 ...
                                 SwipeControllerIpadViewController *newViewController = [[SwipeControllerIpadViewController alloc] init];
...
void rioInterruptionListener1(void *inClientData, UInt32 inInterruption)
{
...
void audioRouteChangeListenerCallback1 (
                                       void                      *inUserData,
                                       ...
                                       const void                *inPropertyValue)
{
    ...
    SwipeControllerIpadViewController *_self = (__bridge SwipeControllerIpadViewController *) inUserData;
...
- (void)viewDidLoad    {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        m_bSwipw.frame = CGRectMake(80.0, 600.0, 600.0, 50.0);
        m_bSwipw.titleLabel.font = [UIFont boldSystemFontOfSize:20.0];
        okbtn.frame = CGRectMake(80.0, 670.0, 200.0, 50.0);
        okbtn.titleLabel.font = [UIFont boldSystemFontOfSize:20.0];
        cancelbtn.frame = CGRectMake(480, 670.0, 200.0, 50.0);
    ...
    amountStatus = [swipe_Status copy];
    ...
    // Get audio units
    status1 = AudioComponentInstanceNew(inputComponent, &audioUnit);
    checkStatus1(status1);
    
    // Enable IO for recording
    UInt32 flag = 1;
    status1 = AudioUnitSetProperty(audioUnit,
    ...
    status1 = AudioSessionGetProperty(kAudioSessionProperty_CurrentHardwareSampleRate, &size, &hwSampleRate);
    checkStatus1(status1);
    NSLog(@"Hardware Sample Rate = %d", (int)hwSampleRate);
    //NSLog(@"Hardware Sample Rate = %d", (int)hwSampleRate);
    
    size = sizeof(audioFormat);
    status1 = AudioUnitGetProperty(audioUnit,
                                  kAudioUnitProperty_StreamFormat,
                                  kAudioUnitScope_Input,
                                  kOutputBus,
                                  &audioFormat,
    ...
    status1 = AudioSessionSetProperty(kAudioSessionProperty_PreferredHardwareIOBufferDuration, sizeof(preferredBufferSize), &preferredBufferSize);
    checkStatus1(status1);
    
    UInt32 maxFPS;
    size = sizeof(maxFPS);
    status1 = AudioUnitGetProperty(audioUnit, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, kOutputBus, &maxFPS, &size);
    checkStatus1(status1);
    ...
    status1 = AudioUnitInitialize(audioUnit);
    checkStatus1(status1);
    ...
    AudioSessionInitialize(NULL, NULL, rioInterruptionListener1, (__bridge void *)(self));
    UInt32 audioCategory = kAudioSessionCategory_PlayAndRecord;
    AudioSessionSetProperty(kAudioSessionProperty_AudioCategory, sizeof(audioCategory), &audioCategory);
    AudioSessionAddPropertyListener (kAudioSessionProperty_AudioRouteChange,
                                     audioRouteChangeListenerCallback1,
                                     (__bridge void *)(self));
    AudioSessionSetActive(true);
...
- (void) finish: (id) sender {
   ...
   // NSLog(@"%d",[amt.text intValue]);
    ...
    if ([name.text isEqualToString:@""] || [cvv.text isEqualToString:@""] || [card_no.text isEqualToString:@""] || [valid.text isEqualToString:@""] || [amt.text isEqualToString:@""]) {
    ...
    else if(amountStatus.intValue < (amt.text).intValue){
        ...
        amountStatus =   [[NSString stringWithFormat:@"%d",amountStatus.intValue-(amt.text).intValue] copy];
        swipe_Status = amountStatus;
        [self doSetKey];
        //SReader_Stop();
...
- (BOOL)SReader_Detect {
    ...
    [self Status1:@"Hold 5 seconds!"];
    sleep(5);
    
    [self Prompt:@"to Initial Card Reader..."];
    [self Status1:@"to Initial Card Reader..."];
...
- (BOOL)SReader_Swipe {
    ...
    TrimePaddingData1(outbuff, &retNum);
    ...
    Decryption_data1 = [[NSString alloc] initWithFormat:@"Decryption data\n%@",
                       formatString];
...
void SReader_Stop1(void)
{
    // to Stop audiounit
    NSLog(@"AudioOutputUnitStop");
    
    status1 = AudioOutputUnitStop(audioUnit);
    checkStatus1(status1);
...
- (void)Display:(NSString *)str {
    ...
    if (str != NULL && ![str isEqualToString:@""]) {
        ...
        [amt performSelectorOnMainThread:@selector(setText:) withObject:amountStatus waitUntilDone:NO];
...
void TrimePaddingData1(unsigned char *outbuff, size_t *retNum)
{
    int len = (int)*retNum;
...
void SReader_Start1(void)
{
    ...
    status1 = AudioOutputUnitStart(audioUnit);
    checkStatus1(status1);
}

void CloseSinWave1(void)
{
    SReader_Stop();
}

void SReader_Init1(void)
{
