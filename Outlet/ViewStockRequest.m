...
@implementation ViewStockRequest
...
- (void)viewDidLoad {
    ...
    outletid_Lbl.text = NSLocalizedString(@"outlet_id", nil);
...
//    toLocationTxt.autocorrectionType = UITextAutocorrectionTypeNo;
    ...
    [locationtxt addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
...
//    deliveryDateTxt.attributedPlaceholder = [[NSAttributedString alloc]initWithString:deliveryDateTxt.placeholder attributes:@{NSForegroundColorAttributeName:[[UIColor lightGrayColor]colorWithAlphaComponent:0.4]}];
    ...
    [f setDateFormat: NSLocalizedString(@"yyyy/MM/dd", nil)];
...
//    }
    ...
    quantityOnHandValueLbl.text = NSLocalizedString(@"0_00", nil);
    previousQtyValueLbl.text    = NSLocalizedString(@"0_00", nil);
    projectedQtyValueLbl.text   = NSLocalizedString(@"0_00", nil);
    requestedQtyvalueLbl.text   = NSLocalizedString(@"0_00", nil);
    approvedQtyValueLbl.text    = NSLocalizedString(@"0_00", nil);
    ...
    @try {
        headerNameLbl.text = NSLocalizedString(@"editStock_request", nil);
...
//    [stockRequestView addSubview:storeIdLbl];
//    [stockRequestView addSubview:toStoreLocTxt];
    ...
    [stockRequestView addSubview:locationtxt];
    ...
    if (currentOrientation == UIDeviceOrientationLandscapeRight || currentOrientation == UIDeviceOrientationLandscapeLeft) {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
           ...
           //productListTbl.frame = CGRectMake( searchItemTxt.frame.origin.x, searchItemTxt.frame.origin.y + searchItemTxt.frame.size.height , searchItemTxt.frame.size.width, 250);
            ...
            quantityOnHandValueLbl.frame = CGRectMake(qohLbl.frame.origin.x + 8,submitBtn.frame.origin.y + 5, qohLbl.frame.size.width,qohLbl.frame.size.height);
...
//        [HUD show:YES];
    ...
    @catch (NSException *exception) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"message", nil) message:NSLocalizedString(@"failed_to_get_the_details", nil) delegate:self cancelButtonTitle: NSLocalizedString(@"OK", nil) otherButtonTitles:nil, nil];
        [alert show];
    }
...
-(void)callingCategoriesList {
    ...
    @try {
        ...
        [locationWiseCategoryDictionary setValue: NSLocalizedString(@"-1", nil) forKey:START_INDEX_STR];
    ...
    @try {
        ...
        if([sender.titleLabel.text isEqualToString:NSLocalizedString(@"edit", nil)] ){
            ...
            if ( (actionRequiredTxt.text).length > 0 ) {
                ...
                conformationAlert = [[UIAlertView alloc] initWithTitle: [NSString stringWithFormat:NSLocalizedString(@"do you want to %@ this indent ", nil),actionRequiredTxt.text ]  message:nil delegate:self cancelButtonTitle:NSLocalizedString(@"yes", nil) otherButtonTitles:NSLocalizedString(@"no", nil), nil];
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"alert" message:@"You can not submit an empty list" delegate:self cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil, nil];
            [alert show];
        }
            ...
            updateStockRequestDic[REQUESTED_BY] = RequestedBy.text;
            
            // Changed by Roja on 02-06-2018....
            if ((toLocationLbl.text).length > 0 && [toLocationLbl.text caseInsensitiveCompare:@"To Warehouse"] == NSOrderedSame && (toLocationTxt.text).length > 0){
                
                updateStockRequestDic[TO_WARE_HOUSE_ID] = toLocationTxt.text;
            }
            
            else if ( (toLocationLbl.text).length > 0 && !
                     ([toLocationLbl.text caseInsensitiveCompare:@"To Warehouse"] == NSOrderedSame) && (toLocationTxt.text).length > 0  ){
...
//            [updateStockRequestDic setObject:requestID forKey:STOCK_REQUEST_ID];
...
- (void)getStockRequestsSuccessResponse:(NSDictionary *)successDictionary {
    ...
    @try {
        ...
        for (NSDictionary * receiptDic in requestsArr) {
            ...
            requestIdTxt.text = [self checkGivenValueIsNullOrNil:[requestViewReceiptJSON  valueForKey:STOCK_REQUEST_ID] defaultReturn:EMPTY_STRING];
            locationtxt.text = [self checkGivenValueIsNullOrNil:[requestViewReceiptJSON  valueForKey:FROM_STORE_CODE] defaultReturn:EMPTY_STRING];
            ...
            if ( [requestViewReceiptJSON.allKeys containsObject:TO_WARE_HOUSE_ID] && ![requestViewReceiptJSON[TO_WARE_HOUSE_ID] isEqualToString: EMPTY_STRING] && ![requestViewReceiptJSON[TO_WARE_HOUSE_ID] isKindOfClass:[NSNull class]] ) {
             
                toLocationLbl.text = NSLocalizedString(@"to_warehouse", nil);
                toLocationTxt.text = [self checkGivenValueIsNullOrNil:[requestViewReceiptJSON  valueForKey:TO_WARE_HOUSE_ID] defaultReturn:EMPTY_STRING];
            }

            else if ( [requestViewReceiptJSON.allKeys containsObject:TO_STORE_CODE] && ![requestViewReceiptJSON[TO_STORE_CODE] isEqualToString: EMPTY_STRING] && ![requestViewReceiptJSON[TO_STORE_CODE] isKindOfClass:[NSNull class]] ) {
                
                toLocationTxt.text = [self checkGivenValueIsNullOrNil:[requestViewReceiptJSON  valueForKey:TO_STORE_CODE] defaultReturn:EMPTY_STRING];
            }
            ...
            totalQtyValueLbl.text = [NSString stringWithFormat:@"%.2f",[requestViewReceiptJSON[TOTAL_STOCK_REQUEST_VALUE] floatValue]];
...
//            for(NSDictionary * tempDic in  [requestViewReceiptJSON objectForKey:STOCK_REQUEST_ITEMS]){
//                [rawMaterialDetails addObject:tempDic];
//            }
...
/**
 * @description  displaying the popOver for the locations......
 * @date         20/0/2016
...
//        
//        
//        if ([locationArr count] < count) {
//            count = [locationArr count];
//            
//        }
//
//        PopOverViewController  *customerInfoPopUp = [[PopOverViewController alloc] init];
//        UIView *customView =[[UIView alloc]initWithFrame:CGRectMake(0.0, 0.0, toStoreLocTxt.frame.size.width,count *  45)];
//        
//        customView.opaque = NO;
//        customView.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:1.0f];
//        customView.layer.borderColor = [UIColor lightGrayColor].CGColor;
...
//            
//            UIPopoverController *popover = [[UIPopoverController alloc] initWithContentViewController:customerInfoPopUp];
//            [popover presentPopoverFromRect:toStoreLocTxt.frame inView:stockRequestView permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
...
//            }
        ...
        @try {
            ...
            priceListDic[START_INDEX] = NSLocalizedString(@"-1", nil);
