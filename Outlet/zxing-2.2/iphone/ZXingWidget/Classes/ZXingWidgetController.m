...
- (id)initWithDelegate:(id<ZXingDelegate>)delegate_ theFrame:(CGRect)theFrame showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode {
    
    self.view.frame = theFrame;
    
    return [self initWithDelegate:delegate_ showCancel:shouldShowCancel OneDMode:shouldUseoOneDMode showLicense:YES];
}
...
- (void)decoder:(Decoder1 *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
...
- (void)decoder:(Decoder1 *)decoder failedToDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset reason:(NSString *)reason {
    decoder.delegate = nil;
    [overlayView setPoints:nil];
}

- (void)decoder:(Decoder1 *)decoder foundPossibleResultPoint:(CGPoint)point {
       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    Decoder1* d = [[Decoder1 alloc] init];
    d.readers = readers;
    d.delegate = self;
