...
unsigned char* LuminanceSource::getMatrix123() {
    int size = width * height;
    unsigned char* result = new unsigned char[size];
    if (left_ == 0 && top_ == 0 && dataWidth_ == width && dataHeight_ == height) {
        memcpy(result, greyData_, size);
    } else {
        for (int row = 0; row < height; row++) {
            memcpy(result + row * width, greyData_ + (top_ + row) * dataWidth_ + left_, width);
        }
    }
    return result;
}
