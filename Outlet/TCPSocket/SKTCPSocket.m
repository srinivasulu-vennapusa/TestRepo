...
- (id)initWithRemoteHost:(NSString *)host port:(int)port {
    if ((self = [super init])) {
        ...
        if (connect(_fileDescriptor, (const struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
            ...
            // if loop created by saradhi ..
            if ([NSException exceptionWithName:SKTCPSocketConnectException reason:@"Failed to connect()." userInfo:nil]) {
...
- (NSData *)readData:(UInt32)length {
    ...
    //NSLog(@" %d",[theData length]);
    //NSLog(@" %@",[NSString stringWithFormat:@"%@", theData]);
    return theData;
}
