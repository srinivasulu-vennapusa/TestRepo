...
#import "WarehousePendingShipment.h"
#import "WHShippingServicesSvc.h"
#import "Global.h"
#import "ViewWarehouseShipment.h"

@interface WarehousePendingShipment ()

@end

@implementation WarehousePendingShipment
@synthesize soundFileObject,soundFileURLRef;

int warePendingShipmentCount1_ = 0;
int warePendingShipmentCount2_ = 1;
int warePendingShipmentCount3_ = 0;
UILabel *recStart;
UILabel *recEnd;
UILabel *totalRec;
UILabel *label1_;
...
BOOL warePendingShipmentCountValue_ = YES;
int warePendingShipmentChangeNum_ = 0;
...
- (void)viewDidLoad {
    ...
    UILabel *titleLbl = [[UILabel alloc] initWithFrame:CGRectMake(110.0, -13.0, 3200.0, 70.0)];
    titleLbl.text = @"Pending Shipments";
    ...
    NSArray *loyaltyObjects = [NSArray arrayWithObjects:[NSString stringWithFormat:@"%d",warePendingShipmentChangeNum_],@"pending",dictionary, nil];
...
- (void) getPreviousOrdersHandler: (NSString *) value {
    ...
    if ([[json objectForKey:@"responseMessage"] isEqualToString:@"ShipmentDetails"] && [[json objectForKey:@"responseCode"] isEqualToString:@"0"]) {
        ...
        recStart.text = [NSString stringWithFormat:@"%d",(warePendingShipmentChangeNum_ * 10) + 1];
        ...
        if ([[JSON1 objectForKey:@"totalShipments"] intValue] <= 10) {
            ...
            if (warePendingShipmentChangeNum_ == 0) {
                previousButton.enabled = NO;
                firstButton.enabled = NO;
                nextButton.enabled = YES;
                lastButton.enabled = YES;
            }
            else if (([[JSON1 objectForKey:@"totalShipments"] intValue] - (10 * (warePendingShipmentChangeNum_+1))) <= 0) {
        ...
        warePendingShipmentCount3_ = [itemIdArray count];
        ...
        if ([listDetails count] == 0) {
            ...
            previousButton.enabled = NO;
            firstButton.enabled = NO;
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Message" message:@"No Pending Shipments" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alert show];
            [alert release];
        }
        ...
        warePendingShipmentCount2_ = NO;
        warePendingShipmentChangeNum_--;
...
-(void) firstButtonPressed:(id) sender {
    ...
    warePendingShipmentChangeNum_ = 0;
    //    cellcount = 10;
...
-(void) lastButtonPressed:(id) sender {
    ...
    if ([totalRec.text intValue]/10 == ([totalRec.text floatValue]/10)) {
        ...
        warePendingShipmentChangeNum_ = [totalRec.text intValue]/10 - 1;
    }
    else{
        warePendingShipmentChangeNum_ =[totalRec.text intValue]/10;
    }
    //changeID = ([rec_total.text intValue]/5) - 1;
    
    //previousButton.backgroundColor = [UIColor grayColor];
    warePendingShipmentCount1_ = (warePendingShipmentChangeNum_ * 10);
...
- (void) previousButtonPressed:(id) sender {
    ...
    if (warePendingShipmentChangeNum_ > 0){
        ...
        warePendingShipmentCountValue_ = NO;
...
- (void) nextButtonPressed:(id) sender {
    ...
    warePendingShipmentCountValue_ = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
   ...
   tableView.separatorColor = [UIColor colorWithRed:66.0/255.0 green:66.0/255.0 blue:66.0/255.0 alpha:1.0];
    ...
    if (tableView == orderstockTable){
        ...
        if (warePendingShipmentCountValue_ == YES) {
            
            warePendingShipmentCount2_ = warePendingShipmentCount2_ + warePendingShipmentCount1_;
            warePendingShipmentCount1_ = 0;
        }
        else{
            
            warePendingShipmentCount2_ = warePendingShipmentCount2_ - warePendingShipmentCount3_;
            warePendingShipmentCount3_ = 0;
        }
