...
//  OutBoundStockRequest.m
//  OmniRetailer
//  Created by Bhargav.v on 5/18/18.
...
@interface OutBoundStockRequest ()

@end

@implementation OutBoundStockRequest
...
- (void)viewDidLoad {
    ...
    saveAllBtn.hidden = YES;
    cancelBtn.hidden = YES;
    generateIndentsBtn.hidden = YES;
    ...
    @try {
        ...
        headerNameLbl.text = NSLocalizedString(@"outbound_stockrequest_summary", nil);
     ...
     // itemApprovedTimeLbl.text = NSLocalizedString(@"approved_on", nil);
        ...
        totalIndentsRequestValueLbl.text = NSLocalizedString(@"0.00", nil);
        totalQuntityValueLbl.text = NSLocalizedString(@"0.00", nil);
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        pagenationTxt.frame = CGRectMake(saveAllBtn.frame.origin.x,cancelBtn.frame.origin.y,90,40);
...
-(void)callingGetStockRequests {
    ...
    @try {
        ...
        if(requestStartNumber == 0)
            ...
            totalQuntityValueLbl.text = @"0.00";
        ...
        if (!isHubLevel) {
            stockRequestDetails[TO_STORE_CODE] = presentLocation;
        }
        else stockRequestDetails[TO_STORE_CODE] = locationStr;
...
//
//            NSString * DraftStr;
//            DraftStr  = [[stockRequestsInfoArr objectAtIndex:sender.tag] valueForKey:STATUS];
//
...
//        if(isDraft){
//
//            newStockRequest.requestID = [[stockRequestsInfoArr objectAtIndex:sender.tag] valueForKey:@"stockRequestId"];
...
-(void)stockRequestDetailsView:(UIButton *)sender {
    ...
    @try {
            ...
            OutBoundViewStockRequest * viewStock = [[OutBoundViewStockRequest alloc] init ];
...
//            NSString * trimmedString = [textField.text stringByTrimmingCharactersInSet:whitespace];
//
//            NSMutableDictionary * dic = [[requestedItemsInfoArr objectAtIndex:textField.tag] mutableCopy];
//
...
//
//            [dic setValue:trimmedString forKey:APPROVED_QTY];
//            [requestedItemsInfoArr replaceObjectAtIndex:textField.tag withObject:dic];
//        }
//    }
    

     
     // Changed by Roja on 04-07-2018...
    ...
    if(tableView == stockRequestTbl) {
        ...
        if (self.isOpen&&self.selectIndex.section == indexPath.section&&indexPath.row!= 0) {
            ...
            @try {
                ...
                if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
                    ...
                    editStockRequestBtn.frame = CGRectMake(actionLbl.frame.origin.x , 0, actionLbl.frame.size.width,hlcell.frame.size.height);
        ...
        @try {
            ...
            @try {
                ...
                //                item_Grade_Lbl.text = [locDic valueForKey:PLU_CODE];
...
//                    qtyChangeTxt.userInteractionEnabled = NO;
                ...
                /*{
                 approvedQty = 46;
                 color = "";
                 itemDesc = "Lotus Neem Clear 25 G";
                 model = NA;
                 pluCode = "--";
                 quantity = 2;
                 size = "--";
                 skuId = 000;
                 sno = 405;
                 stockRequestId = SR20160901175932;
                 totalCost = 0;
                 }
                 */
            } @catch (NSException *exception) {
...
//        NSString * quoteRequestJsonString   = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
        ...
        // till here added by roja on 04-07-2018.....
...
-(void)callingBrandList {
    @try {
        ...
        [brandDic setValue:@"-1" forKey:START_INDEX];
...
- (void)showCompleteStockRequestInfo:(UIButton *)sender {
    ...
    @try {
        ...
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
            ...
            //  ustomerInfoPopUp.contentSizeForViewInPopover = CGSizeMake(160.0, 260.0);
