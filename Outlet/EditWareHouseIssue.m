...
@interface EditWareHouseIssue ()

@end

@implementation EditWareHouseIssue

@synthesize soundFileObject,soundFileURLRef;

int wareEditQuantity_issue = 0;
int wareEditMaterialTagid_issue = 0;
float wareEditMaterialCost_issue = 0.0f;
int wareEditRejectMaterialTagid_issue = 0;
int wareReceipt_id_val_int;
NSString *wareEditReceipt_issue = @"";
NSString *wareEditFinalReceiptID_issue = @"";
NSString *wareIssueIdStr = @"";
int wareEditlocationIndex = 0;

-(id) initWithReceiptID:(NSString *)receiptID{
    
    wareEditFinalReceiptID_issue = [receiptID copy];
...
- (void)viewDidLoad {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        totalQunatity.font = [UIFont boldSystemFontOfSize:25];
        ...
        totalCost.font = [UIFont boldSystemFontOfSize:25];
    ...
    [self callReceiptIDDetails:wareEditFinalReceiptID_issue];

}

-(void) callReceiptIDDetails:(NSString *)receiptID{
    
    wareEditMaterialCost_issue = 0.0f;
    wareEditQuantity_issue = 0;
    ...
    [receiptDetails1 setObject:wareEditFinalReceiptID_issue forKey:@"goods_issue_ref_num"];
    ...
    if (status) {
        NSDictionary *temp1 = [JSONData valueForKey:@"warehouse_issueDetails"];
        // NSLog(@"%@",temp1);
        ...
        wareReceipt_id_val_int = [[temp1 objectForKey:@"id_goods_issue"] intValue];
        deliveredBy.text = [temp1 objectForKey:@"delivered_by"];
        inspectedBy.text = [temp1 objectForKey:@"inspectedBy"];
        toLocation.text = [temp1 objectForKey:@"issue_to"];
        ...
        for (int j = 0; j < [rawMaterialDetails count]; j++) {
            ...
            wareEditQuantity_issue = wareEditQuantity_issue + [[temp objectAtIndex:5] intValue];
            wareEditMaterialCost_issue = wareEditMaterialCost_issue + ([[temp objectAtIndex:5] intValue] * [[temp objectAtIndex:2]  floatValue]);
        }
        
        totalQunatity.text = [NSString stringWithFormat:@"%d",wareEditQuantity_issue];
        totalCost.text = [NSString stringWithFormat:@"%.2f",wareEditMaterialCost_issue];
...
-(void) callRawMaterialDetails:(NSString *)rawMaterial{
    ...
    NSArray *temp = [NSArray arrayWithObjects:[JSON objectForKey:@"description"],[JSON objectForKey:@"productName"],[JSON objectForKey:@"price"],@"1",@"0",@"1",@"0", [JSON objectForKey:@"price"],nil];
    ...
    for (int c = 0; c < [rawMaterialDetails count]; c++) {
        ...
        if ([[material objectAtIndex:0] isEqualToString:[NSString stringWithFormat:@"%@",[JSON objectForKey:@"productName"]]]) {
            ...
            NSArray *temp = [NSArray arrayWithObjects:[material objectAtIndex:0],[material objectAtIndex:1],[material objectAtIndex:2],[NSString stringWithFormat:@"%d",[[material objectAtIndex:3] intValue] + 1],@"0",[NSString stringWithFormat:@"%d",[[material objectAtIndex:5] intValue]+1],[material objectAtIndex:6],[NSString stringWithFormat:@"%.2f",(([[material objectAtIndex:3] intValue] + 1) * [[material objectAtIndex:2] floatValue])], nil];
    ...
    for (int i = 0; i < [rawMaterialDetails count]; i++) {
        ...
        wareEditQuantity_issue = wareEditQuantity_issue + [[material objectAtIndex:5] intValue];
        wareEditMaterialCost_issue = wareEditMaterialCost_issue + ([[material objectAtIndex:5] intValue] * [[material objectAtIndex:2] floatValue]);
    }
...
- (void)textFieldDidChange:(UITextField *)textField {
    if (textField == BillField) {
        ...
        wareEditRejectMaterialTagid_issue = textField.tag;
        ...
        wareEditMaterialTagid_issue = textField.tag;
...
-(void)getListOfLocations:(id)sender {
    ...
    if (sender == selectLocation) {
        ...
        wareEditlocationIndex = 0;
        [self getLocations:wareEditlocationIndex];
...
- (IBAction)okButtonPressed:(id)sender {
    ...
    NSArray *temp = [rawMaterialDetails objectAtIndex:wareEditMaterialTagid_issue];
    ...
    if (qty > [[temp objectAtIndex:7] intValue]){
        ...
        NSArray *finalArray = [NSArray arrayWithObjects:[temp objectAtIndex:0],[temp objectAtIndex:1],[NSString stringWithFormat:@"%d",qty],[NSString stringWithFormat:@"%d", qty],[NSString stringWithFormat:@"%d",qty],@"0",[temp objectAtIndex:6],[temp objectAtIndex:7], nil];
        
        [rawMaterialDetails replaceObjectAtIndex:wareEditMaterialTagid_issue withObject:finalArray];
...
- (void)rejecQtyOkButtonPressed:(id)sender{
    ...
    NSArray *temp = [rawMaterialDetails objectAtIndex:wareEditRejectMaterialTagid_issue];
    
    if (qty > [[temp objectAtIndex:5] intValue]){
    ...
    //    }
        ...
        NSArray *finalArray = [NSArray arrayWithObjects:[temp objectAtIndex:0],[temp objectAtIndex:1],[temp objectAtIndex:2],[temp objectAtIndex:3],@"0",[NSString stringWithFormat:@"%d",received],[NSString stringWithFormat:@"%d",qty],[temp objectAtIndex:7], nil];
        
        [rawMaterialDetails replaceObjectAtIndex:wareEditRejectMaterialTagid_issue withObject:finalArray];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == skListTable) {
        ...
        locationTable.hidden = YES;
        toLocation.text = [locationArr objectAtIndex:indexPath.row];
...
-(void)changeQuantity:(UIButton *)sender{
    ...
    wareEditRejectMaterialTagid_issue = sender.tag;
...
-(void)submitButtonPressed{
    ...
    if ([rawMaterialDetails count] == 0){
        ...
        for (int i = 0; i < [rawMaterialDetails count]; i++) {
            ...
            [cost addObject:[temp objectAtIndex:7]];
            //            [make addObject:[temp objectAtIndex:5]];
            [supplied addObject:[temp objectAtIndex:5]];
        ...
        [receiptDetails1 setObject:[NSString stringWithFormat:@"%d",wareReceipt_id_val_int] forKey:@"id_goods_issue"];
        [receiptDetails1 setObject:totalQunatity.text forKey:@"issue_total_qty"];
        ...
        WHStockIssueServices_updateStockIssue *create_receipt = [[WHStockIssueServices_updateStockIssue alloc] init];
        create_receipt.stockIssueDetails = createReceiptJsonString;
        
        WHStockIssueServicesSoapBindingResponse *response = [materialBinding updateStockIssueUsingParameters:create_receipt];
        NSArray *responseBodyParts1 = response.bodyParts;
        
        for (id bodyPart in responseBodyParts1) {
            
            if ([bodyPart isKindOfClass:[WHStockIssueServices_updateStockIssueResponse class]]) {
                
                
                [HUD_ setHidden:YES];
                WHStockIssueServices_updateStockIssueResponse *body = (WHStockIssueServices_updateStockIssueResponse *)bodyPart;
                printf("\nresponse=%s",[body.return_ UTF8String]);
                ...
                if (json1!=NULL) {
                    ...
                    json_issue = [json1 objectForKey:@"responseHeader"] ;
                    if ([[json_issue objectForKey:@"responseMessage"] isEqualToString:@"Success"] && [[json_issue objectForKey:@"responseCode"] isEqualToString:@"0"]) {
                        ...
                        wareIssueIdStr = [receiptID copy];
                        UIAlertView *successAlertView  = [[UIAlertView alloc] init];
                        [successAlertView setDelegate:self];
                        [successAlertView setTitle:@"Material  Issue Submitted Successfully"];
        ...
        for (id bodyPart in responseBodyParts1) {
            ...
            if ([bodyPart isKindOfClass:[WHStockIssueServices_updateStockIssueResponse class]]) {
                ...
                if (json1!=NULL) {
                    ...
                    json_issue = [json1 objectForKey:@"responseHeader"];
                    if ([[json_issue objectForKey:@"responseMessage"] isEqualToString:@"Success"]) {
                        
                        NSString *receiptID = [json1 objectForKey:@"issueid"];
                        wareIssueIdStr = [receiptID copy];
                        UIAlertView *successAlertView  = [[UIAlertView alloc] init];
                        [successAlertView setDelegate:self];
                        [successAlertView setTitle:@"Material  Issue Saved Successfully"];
                        [successAlertView setMessage:[NSString stringWithFormat:@"%@%@",@"Receipt ID : ",receiptID]];
                        [successAlertView addButtonWithTitle:@"OPEN RECEIPT"];
                        [successAlertView addButtonWithTitle:@"NEW RECEIPT"];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if ([alertView.title isEqualToString:@"Material  Issue Submitted Successfully"]) {
        if (buttonIndex == 0) {
            ...
            OpenWareHouseIssue *stockReceipt = [[OpenWareHouseIssue alloc] initWithReceiptID:wareEditFinalReceiptID_issue];
