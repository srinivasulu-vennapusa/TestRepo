...
@interface OpenWareHouseReceipt ()

@end

@implementation OpenWareHouseReceipt

@synthesize soundFileURLRef,soundFileObject;

NSString *wareFinalReceiptID_ = @"";
NSDictionary *wareViewReceiptJSON_ = NULL;

int wareStockQuantity = 0;
float wareStockMaterialCost = 0.0f;

-(id) initWithReceiptID:(NSString *)receiptID{
    
    wareFinalReceiptID_ = [receiptID copy];
...
-(void)callReceiptDetails{
    ...
    [receiptDetails1 setObject:wareFinalReceiptID_ forKey:@"goods_receipt_ref_num"];
        ...
        if ([bodyPart isKindOfClass:[WHStockReceiptService_getStockReceiptResponse class]]) {
            ...
            JSONData = [[NSJSONSerialization JSONObjectWithData: [body.return_ dataUsingEncoding:NSUTF8StringEncoding]
                                                        options: NSJSONReadingMutableContainers
                                                          error: &e] copy];
            wareViewReceiptJSON_ = [JSONData copy];
        }
    }
    if (status_) {
        NSDictionary *temp1 = [wareViewReceiptJSON_ objectForKey:@"warehouse_receipt"];
        NSLog(@"%@",temp1);
        
        NSArray *items = [wareViewReceiptJSON_ objectForKey:@"itemDetails"];
        ...
        locationValue.text = [temp1 objectForKey:@"shipped_from"];
        ...
        wareStockQuantity = 0;
        wareStockMaterialCost = 0.0;
        
        for (int j = 0; j < [itemDetails count]; j++) {
            
            NSArray *temp = [itemDetails objectAtIndex:j];
            
            wareStockQuantity = wareStockQuantity + [[temp objectAtIndex:8] intValue];
            wareStockMaterialCost = wareStockMaterialCost + ([[temp objectAtIndex:8] intValue] * [[temp objectAtIndex:4]  floatValue]);
        }
        
        totalQuantity.text = [NSString stringWithFormat:@"%d",wareStockQuantity];
        totalCost.text = [NSString stringWithFormat:@"%.2f",wareStockMaterialCost];
...
- (void)viewDidLoad {
    ...
    titleLbl.text = @"Stock Receipt Details";
...
-(void) buttonClicked1:(UIButton*)sender
{
    ...
    if (sender.tag == 0) {
        ...
        if ([statusValue.text isEqualToString:@"Pending"]) {
            ...
            EditWareHouseReceipt *editReceipt = [[EditWareHouseReceipt alloc] initWithReceiptID:wareFinalReceiptID_];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == cartTable){
        ...
        item_description.text = [NSString stringWithFormat:@"%@",[temp objectAtIndex:3]];
        ...
        price.text = [NSString stringWithFormat:@"%.2f",[[temp objectAtIndex:4] floatValue]];
        ...
        [qtyButton setTitle:[NSString stringWithFormat:@"%d",[[temp objectAtIndex:5] integerValue]] forState:UIControlStateNormal];
        ...
        cost.text = [NSString stringWithFormat:@"%.02f", [[temp objectAtIndex:10] floatValue]];
        ...
        supplied.text = [NSString stringWithFormat:@"%d",[[temp objectAtIndex:7] intValue]];
        ...
        received.text = [NSString stringWithFormat:@"%d",[[temp objectAtIndex:8] intValue]];
        ...
        [rejectQtyButton setTitle:[NSString stringWithFormat:@"%d",[[temp objectAtIndex:9] intValue]] forState:UIControlStateNormal];
