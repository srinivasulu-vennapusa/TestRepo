...
- (void)viewDidLoad
{
    ...
    self.titleLabel.text = @"Gift Coupon Details";
    ...
    loyaltyNumtxt.placeholder = @"Scan or enter Coupon no or Mobile No.";  //place holder
    ...
    label.text = @" Please Enter Coupon Number or Mobile No.";
    ...
    noOfVouchersLbl.text = @"Coupon Id";
    ...
    expiryDateLbl.text = @"Coupon Value";
    ...
    voucherNumberLbl.text = @"Coupon No.";
    ...
    noVouchersLbl.text = @"No Unclaimed Coupons for this Customer";
...
//        GiftCouponServicesSvc_getCouponDetails *voucher = [[GiftCouponServicesSvc_getCouponDetails alloc] init];
//
//        voucher.couponDetails = loyaltyString;
//        @try {
//
//            GiftCouponServicesSoapBindingResponse *response = [skuService getCouponDetailsUsingParameters:voucher];
...
- (void) submitButtonPressed:(id) sender {
    ...
    if (value.length == 0){
        ...
        services.giftCouponServicesDelegate = self;
        [services getGiftCouponDetails:loyaltyString];
    }
}

// added by Roja on 17/10/2019…. // OLD code only added belwo...
...
// Method definition chnaged by roja on 17/10/2019...
- (void) getissuedLoyaltycardHandler: (NSDictionary *) successReponse {
...
//    if ([[[JSON valueForKey:RESPONSE_HEADER] valueForKey:RESPONSE_CODE] intValue] == 0 && [[[JSON valueForKey:RESPONSE_HEADER] valueForKey:RESPONSE_MESSAGE] isEqualToString:@"Success"]) {
        ...
        if(successReponse != nil){
            
        loyaltyNumtxt.text = nil;
        username.text = @"Coupon Prg Code";
        phNo.text = @"Coupon Code";
        email.text = @"Coupon ID";
        validFrom.text = @"Valid From";
        validThru.text = @"Valid To";
        availPoints.text = @"Coupon Value";
        ...
        if ([[successReponse valueForKey:@"customerGiftCoupons"] isKindOfClass:[NSNull class]]) {
            ...
            NSDictionary *gvDic = [successReponse valueForKey:@"giftcoupons"];
            
            if ([[gvDic valueForKey:@"couponProgramCode"] isKindOfClass:[NSNull class]]) {
                usernameData.text = @"--";
            }
            else {
                usernameData.text = [NSString stringWithFormat:@"%@",gvDic[@"couponProgramCode"]];
            }
            
            if ([[gvDic valueForKey:@"couponCode"] isKindOfClass:[NSNull class]]) {
                phNoData.text = @"--";
            }
            else {
                phNoData.text = [NSString stringWithFormat:@"%@",gvDic[@"couponCode"]];
            }
            
            if ([[gvDic valueForKey:@"couponId"] isKindOfClass:[NSNull class]]) {
                emailData.text = @"--";
            }
            else {
                emailData.text = [NSString stringWithFormat:@"%@",gvDic[@"couponId"]];
            }
            
            NSDictionary *gvDetailsDic = [successReponse valueForKey:@"giftcouponDetails"];
            ...
            if ([[successReponse valueForKey:@"claimStatus"] boolValue]) {
                amountData.text = @"YES";
            }
            else {
                amountData.text = @"NO";
            ...
            NSArray *customerGiftVouchers = [successReponse valueForKey:@"customerGiftcoupons"];
...
//        UIAlertView *validalert = [[UIAlertView alloc] initWithTitle:[[JSON valueForKey:RESPONSE_HEADER] valueForKey:RESPONSE_MESSAGE] message:nil delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
        ...
        UIAlertView *validalert = [[UIAlertView alloc] initWithTitle:@"Unable to get Coupon details" message:nil delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [validalert show];
    }
...
// so, latest changes done in another method with same method name(getissuedLoyaltycardHandler:)
...
//        AudioServicesPlaySystemSound (soundFileObject1);
//
//        if ([[JSON valueForKey:@"customerGiftCoupons"] isKindOfClass:[NSNull class]]) {
//
//            validFrom.hidden = NO;
//            validThru.hidden = NO;
//            availPoints.hidden = NO;
...
//            noVouchersLbl.hidden = YES;
//
//            NSDictionary *gvDic = [JSON valueForKey:@"giftcoupons"];
//
//            if ([[gvDic valueForKey:@"couponProgramCode"] isKindOfClass:[NSNull class]]) {
//                usernameData.text = @"--";
//            }
...
//            }
//
//            NSDictionary *gvDetailsDic = [JSON valueForKey:@"giftcouponDetails"];
//
//            if ([[gvDetailsDic valueForKey:@"createdOn"] isKindOfClass:[NSNull class]]) {
//                validFromData.text = @"--";
//            }
...
//        }
//        else {
//            NSArray *customerGiftVouchers = [JSON valueForKey:@"customerGiftcoupons"];
//
//            if (customerGiftVouchers.count > 0) {
//                NSDictionary *custGVInfoDic = customerGiftVouchers[0];
//                username.text = @"Customer Name";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @try {
        ...
        if ([[dic valueForKey:@"couponCode"] isKindOfClass:[NSNull class]]) {
            skid.text = @"--";
        }
        else {
            skid.text = [NSString stringWithFormat:@"%@",[dic valueForKey:@"couponCode"]];
        }
        ...
        if ([[dic valueForKey:@"couponId"] isKindOfClass:[NSNull class]]) {
            mrpPrice.text = @"--";
        }
        else {
            mrpPrice.text = [NSString stringWithFormat:@"%@",[dic valueForKey:@"couponId"]];
        }
        ...
        if ([[dic valueForKey:@"couponValue"] isKindOfClass:[NSNull class]]) {
            price.text = @"--";
        }
        else {
            price.text = [NSString stringWithFormat:@"%.2f",[[dic valueForKey:@"couponValue"] floatValue]];
        }
