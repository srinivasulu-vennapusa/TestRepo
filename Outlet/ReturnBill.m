...
//  OmniRetailer
//
//  Created by Bhargav on 11/16/16.
//
//
...
int return_bill_no = 0;

@interface ReturnBill ()

@end

@implementation ReturnBill
...
- (void)viewDidLoad {
    ...
    returnBillView = [[UIView alloc] init];
    returnBillView.backgroundColor = [UIColor blackColor];
    returnBillView.layer.borderWidth = 1.0f;
    returnBillView.layer.cornerRadius = 10.0f;
    returnBillView.layer.borderColor = [UIColor clearColor].CGColor;
    ...
    returnBillField = [[UITextField alloc] init];
    returnBillField.borderStyle = UITextBorderStyleRoundedRect;
    returnBillField.textColor = [UIColor blackColor];
    returnBillField.backgroundColor = [[UIColor whiteColor]colorWithAlphaComponent:0.7 ];
    returnBillField.clearButtonMode = UITextFieldViewModeWhileEditing;
    returnBillField.autocorrectionType = UITextAutocorrectionTypeNo;
    returnBillField.returnKeyType = UIReturnKeyDone;
    [returnBillField addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
    returnBillField.delegate = self;
    returnBillField.placeholder = NSLocalizedString(@"search_bill", nil);
    ...
    returnBillIdTbl = [[UITableView alloc] init];
    returnBillIdTbl.dataSource = self;
    returnBillIdTbl.delegate = self;
    returnBillIdTbl.backgroundColor = [UIColor clearColor];
    returnBillIdTbl.tableFooterView = [[UIView alloc] initWithFrame:CGRectZero];
    ...
    [self.view addSubview:returnBillView];
    
    [returnBillView addSubview:summaryInfoBtn];
    [returnBillView addSubview:returnBillField];
    [returnBillView addSubview:startDateField];
    [returnBillView addSubview:billDteButton];
    [returnBillView addSubview:endDateField];
    [returnBillView addSubview:endDateButton];
    [returnBillView addSubview:userMobileNoFld];
    [returnBillView addSubview: submitBtn];
    
    
    //changed by Srinivasulu on 07/08/2017....
    
    //    adding labels to the return bill view:
    //    [returnBillView addSubview:SnoLbl];
    //    [returnBillView addSubview:BillIdLbl];
    //    [returnBillView addSubview:dateLbl];
    //    [returnBillView addSubview:counterLbl];
    //    [returnBillView addSubview:userNameLbl];
    //    [returnBillView addSubview:billDueLbl];
    //
    //    [returnBillView addSubview:returnBillIdTbl];
    ...
    [itemsScrollView addSubview:returnBillIdTbl];
    
    [returnBillView addSubview:itemsScrollView];
    
    [returnBillView addSubview:pagenationTxt];
    [returnBillView addSubview:dropDownBtn];
    [returnBillView addSubview:goButton];
    
    //Adding the SubView for the returnBill View to display the TotalRecords count...
    [returnBillView addSubview:totalRecordsView];
    ...
    @try {
        ...
        self.titleLabel.text = NSLocalizedString(@"return_billing", nil);
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        if (currentOrientation == UIDeviceOrientationLandscapeRight || currentOrientation == UIDeviceOrientationLandscapeLeft) {
            ...
            @try {
                returnBillView.frame = CGRectMake( 2, 70, self.view.frame.size.width - 4, self.view.frame.size.height - 80);
                summaryInfoBtn.frame = CGRectMake(returnBillView.frame.size.width - 58,10, 40, 30);
                
                returnBillField  .font = [UIFont boldSystemFontOfSize:20];
                returnBillField.frame = CGRectMake(returnBillView.frame.origin.x+20, summaryInfoBtn.frame.origin.y+40, 250, 40);
                
                userMobileNoFld.font = [UIFont boldSystemFontOfSize:20];
                userMobileNoFld.frame = CGRectMake(returnBillField.frame.origin.x+returnBillField.frame.size.width+65 , returnBillField.frame.origin.y, 180, 40);
                
                startDateField.font = [UIFont boldSystemFontOfSize:20];
                
                startDateField.frame = CGRectMake(userMobileNoFld.frame.origin.x+userMobileNoFld.frame.size.width+15 , returnBillField.frame.origin.y, 180, 40);
                ...
                endDateField.frame = CGRectMake(startDateField.frame.origin.x+startDateField.frame.size.width+15 , returnBillField.frame.origin.y, 180, 40);
                ...
                //changed by Srinivasulu on 07/08/2017....
                itemsScrollView.frame = CGRectMake( returnBillField.frame.origin.x, returnBillField.frame.origin.y + returnBillField.frame.size.height+10, (summaryInfoBtn.frame.origin.x + summaryInfoBtn.frame.size.width) - returnBillField.frame.origin.x, returnBillView.frame.size.height - (returnBillField.frame.origin.y + returnBillField.frame.size.height  + 60));
                ...
                returnBillIdTbl.frame = CGRectMake( 0, SnoLbl.frame.origin.y + SnoLbl.frame.size.height + 4, completeWidth, itemsScrollView.frame.size.height - ( SnoLbl.frame.origin.y + SnoLbl.frame.size.height + 4));
                
                itemsScrollView.contentSize = CGSizeMake( completeWidth, itemsScrollView.frame.size.height);
                
                // Added By Bhargav.v on 23/02/2018...
                
                pagenationTxt.frame = CGRectMake(returnBillField.frame.origin.x,itemsScrollView.frame.origin.y + itemsScrollView.frame.size.height + 10,90,40);
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    @try {
        if (returnBillIdsArr ==  nil && returnBillIdsArr.count == 0) {
            [HUD setHidden:NO];
            ...
            if(!isOfflineService){
                ...
                [self getReturnedBillsinOffline];
            }
...
-(void)getReturnBills {
    ...
    @try {
        ...
        if(returnBillIdsArr == nil && return_bill_no == 0){
            
            totalNumberOfRecords = 0;
            
            returnBillIdsArr = [NSMutableArray new];
        }
        else if(returnBillIdsArr.count) {
            
            [returnBillIdsArr removeAllObjects];
        }
        
        WebServiceController *service = [[WebServiceController alloc] init];
        service.getBillsDelegate = self;
        [service getBills:return_bill_no deliveryType:@"" status:@"Returned"];
    }
    @catch (NSException *exception) {
        
        [HUD setHidden:YES];
        float y_axis = self.view.frame.size.height - 450;
        NSString * mesg = [NSString stringWithFormat:@"%@%@%@",NSLocalizedString(@"alert", nil),@"\n",NSLocalizedString(@"returned_bills_are_not_availabe", nil)];
    ...
    //
    //                NSArray *billsList = [offline getReturnedBills:@""];
    //
    //                for (NSDictionary *dic in billsList) {
    //
    //                    [returnBillIdsArr addObject:dic];
    //
    //                }
    ////                if ([returnBillIdsArr count]) {
    ////                    [returnBillIdsArr removeLastObject];
    ////                }
    //                [returnBillIdTbl reloadData];
    //
    //
    //                [HUD setHidden:YES];
    //
    ...
    //
    //                    NSString * mesg = [NSString stringWithFormat:@"%@%@%@",NSLocalizedString(@"alert", nil),@"\n",NSLocalizedString(@"return_bills_not_availabe", nil)];
    //
...
-(void)getBillsSuccesResponse:(NSDictionary *)successDictionary {
    @try {
        ...
        [returnBillIdTbl setUserInteractionEnabled:YES];
...
-(void)getBillsFailureResponse:(NSString *)failureString {
    ...
    @finally {
        ...
        if( return_bill_no == 0 || returnBillIdsArr.count == 0 ) {
...
-(void)getReturnedBillsinOffline {
    ...
    @try {
        ...
        OfflineBillingServices * offline = [[OfflineBillingServices alloc] init];
        NSMutableDictionary * successDictionary = [offline getBillInfo:RETURNED_OR_ITEMS searchInfo:@""  mobileNo:userMobileNoFld.text startingDate:startDteStr endDate:endDteStr startIndex:return_bill_no maxRecords:10];
...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    ...
    @try {
        ...
        [returnBillField resignFirstResponder];
        return YES;
...
- (void)textFieldDidChange:(UITextField *)textField {
    ...
    if(textField == returnBillField) {
        
        @try {
            
            NSString *value = [returnBillField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            
            saleId = returnBillField.text;
            ...
            if (saleId.length == 3 && value.length != 0) {
                ...
                if (!isOfflineService) {
                    ...
                    //added by Srinivasulu on 13/06/2017....
                    returnBillField.tag = 4;
                    //upto here on 13/06/2017....
                    
                    WebServiceController *controller = [[WebServiceController alloc] init];
                    controller.getBillsDelegate = self;
                    [controller getBillIds:-1 deliveryType:@"" status:@"Returned" searchCriteria:saleId];
                    //                Exchanged
                    //                Returned
                    ...
                    OfflineBillingServices *offline = [[OfflineBillingServices alloc]init];
                    NSMutableArray *result = [offline getReturnedBills:returnBillField.text];
                    [HUD setHidden:YES];
                ...
                if (!isOfflineService) {
                    ...
                    @try {
                        ...
                        {
                            NSComparisonResult result = [product compare:returnBillField.text options:(NSCaseInsensitiveSearch|NSDiacriticInsensitiveSearch) range:NSMakeRange(0, (returnBillField.text).length)];
            ...
            if(filteredSkuArrayList.count && (saleId.length >= 3)){
                ...
                [self showPopUpForTables:billIdsTbl  popUpWidth:returnBillField.frame.size.width  popUpHeight:tableHeight presentPopUpAt:returnBillField  showViewIn:self.view permittedArrowDirections:UIPopoverArrowDirectionUp];
            }
            else
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    @try {
        ...
        if (tableView == returnBillIdTbl) {
            return returnBillIdsArr.count;
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == returnBillIdTbl || tableView == pagenationTbl){
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
 ...
 // Commented By Bhargav.v on 23/02/2018...
 // Reason: Handling the Pagenation Based on go Button instead of Scrolling the TableView for every 10 records.....
...
//        @try {
//            
//            if((indexPath.row == ([returnBillIdsArr count] - 1))  && ([returnBillIdsArr count] < totalNumberOfRecords)){
//                // [HUD show: YES];
//                [HUD setHidden:NO];
//                return_bill_no = return_bill_no + 10;
//                
//                if(!isOfflineService){
//                    
//                    if(submitBtn.tag == 2 )
    ...
    if (tableView == returnBillIdTbl) {
        ...
        @try {
            ...
            @try {
                ...
                NSDictionary * dic = returnBillIdsArr[indexPath.row];
                
                s_no_Lbl.text = [NSString stringWithFormat:@"%ld", (indexPath.row + 1) + return_bill_no];
    ...
    @try {
        ...
        if (tableView == billIdsTbl) {
            ...
            @try {
                ...
                return_bill_no = return_bill_no + (pageValue * 13) - 13;
            ...
            NSDictionary *detailsDic = returnBillIdsArr[indexPath.row];
            
            // past_bill = [[PastBilling alloc]initWithBillType:[detailsDic valueForKey:BILL_ID]];
        ...
        past_bill.billTypeStr = NSLocalizedString(@"returned_bill", nil);
...
-(void)callingSearchBills {
    @try {
        ...
        NSLog(@"---getPastBills---%d",return_bill_no);
        ...
        else if(return_bill_no == 0 &&  returnBillIdsArr.count ){
        ...
        NSArray * headerObjects_ = @[[RequestHeader getRequestHeader],presentLocation,[NSString stringWithFormat:@"%d",return_bill_no],userMobileNoFld.text,startDteStr,endDteStr,@"Returned",@"10"];
...
- (void)showCalenderInPopUp:(UIButton *)sender {
    ...
    @try {
        ...
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
            ...
            if(sender.tag == 2)
                [popover presentPopoverFromRect:startDateField.frame inView:returnBillView permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
            else
                [popover presentPopoverFromRect:endDateField.frame inView:returnBillView permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
...
-(void)showPaginationData:(UIButton *)sender{
    ...
    @try {
        ...
        [self showPopUpForTables:pagenationTbl  popUpWidth:pagenationTxt.frame.size.width  popUpHeight:tableHeight presentPopUpAt:pagenationTxt  showViewIn:returnBillView permittedArrowDirections:UIPopoverArrowDirectionLeft];
