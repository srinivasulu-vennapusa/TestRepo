//
//  ViewWareHouseProcurementReceipt.m
//  OmniRetailer
//
...
@interface ViewWareHouseProcurementReceipt ()

@end

@implementation ViewWareHouseProcurementReceipt

@synthesize soundFileURLRef,soundFileObject;

NSString *wareFinalProcurementReceiptID = @"";
NSDictionary *wareViewReceiptJSON = NULL;

-(id) initWithReceiptID:(NSString *)receiptID{
    
    wareFinalProcurementReceiptID = [receiptID copy];
...
-(void)callReceiptDetails {
    ...
    NSArray *headerObjects_ = [NSArray arrayWithObjects:wareFinalProcurementReceiptID,[RequestHeader getRequestHeader], nil];
            ...
            if ([bodyPart isKindOfClass:[WHStockReceiptService_getStockProcurementReceiptResponse class]]) {
                ...
                JSONData = [[NSJSONSerialization JSONObjectWithData: [body.return_ dataUsingEncoding:NSUTF8StringEncoding]
                                                            options: NSJSONReadingMutableContainers
                                                              error: &e] copy];
                wareViewReceiptJSON = [JSONData copy];
            }
        }
    }
    ...
    if (status_) {
        ...
        dateValue.text = [wareViewReceiptJSON objectForKey:@"date"];
        receiptRefNoValue.text = [wareViewReceiptJSON objectForKey:@"goods_receipt_ref_num"];
        supplierIDValue.text = [wareViewReceiptJSON objectForKey:@"supplier_id"];
        supplierNameValue.text = [wareViewReceiptJSON objectForKey:@"supplier_name"];
        locationValue.text = [wareViewReceiptJSON objectForKey:@"location"];
        shipmentValue.text = [wareViewReceiptJSON objectForKey:@"shipment_note"];
        deliveredBYValue.text = [wareViewReceiptJSON objectForKey:@"delivered_by"];
        inspectedBYValue.text = [wareViewReceiptJSON objectForKey:@"inspected_by"];
        poRefValue.text = [wareViewReceiptJSON objectForKey:@"po_reference"];
        totalQuantity.text = [NSString stringWithFormat:@"%@",[wareViewReceiptJSON objectForKey:@"receipt_total_qty"]];
        totalCost.text = [NSString stringWithFormat:@"%.2f",[[wareViewReceiptJSON objectForKey:@"grand_total"] floatValue]];
        
        if ([[wareViewReceiptJSON objectForKey:@"status"] isEqualToString:@"false"]) {
            statusValue.hidden = NO;
            status.hidden = NO;
            statusValue.text = @"Pending";
        }
        
        NSArray *items = [wareViewReceiptJSON objectForKey:@"items"];
        
        for (int i = 0; i < [items count]; i++) {
            
            [itemDetails addObject:[items objectAtIndex:i]];
        }
...
-(void) buttonClicked1:(UIButton*)sender
{
    ...
    if (sender.tag == 0) {
        ...
        if (![statusValue.text isEqualToString:@"Pending"]) {
            ...
            EditWareHouseProcurementReceipt *editReceipt = [[EditWareHouseProcurementReceipt alloc] initWithReceiptID:wareFinalProcurementReceiptID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == cartTable){
        ...
        NSDictionary *temp = [itemDetails objectAtIndex:indexPath.row];
