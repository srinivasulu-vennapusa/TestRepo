...
@interface WareHouseStockIssue ()

@end

@implementation WareHouseStockIssue

@synthesize soundFileURLRef,soundFileObject;

int wareReceiptMaterialQuantity = 0;
int wareReceieptMaterialTagid = 0;
int wareReceiptRejectMaterialTagId = 0;
float wareReceiptMaterialIssueCost = 0.0f;
int warehStartPoint;
NSString *warehReceipt;
int  wareStartIndexInt = 0;
NSDictionary *wareJSON_issue = NULL;
bool wareScrollValueIssueStatus = NO;

bool wareTolocationScrollValueStatus_ = NO;
int  wareTolocationStartIndex = 0;

/**
 
 @function       callRawMaterialDetails
 @abstract       get the raw material details of the selected skuId
 @param          raw material name (NSString *)
...
-(void) callRawMaterialDetails:(NSString *)rawMaterial {
    ...
    @try {
        ...
        for (id bodyPart in responseBodyParts) {
            if ([bodyPart isKindOfClass:[SkuServiceSvc_getSkuDetailsResponse class]]) {
                ...
                NSError *e;
                wareJSON_issue = [NSJSONSerialization JSONObjectWithData: [body.return_ dataUsingEncoding:NSUTF8StringEncoding]
    ...
    NSArray *temp = [NSArray arrayWithObjects:[wareJSON_issue objectForKey:@"description"],[wareJSON_issue objectForKey:@"productName"],[wareJSON_issue objectForKey:@"price"],@"1",[wareJSON_issue objectForKey:@"price"],@"N/A",@"1",@"1",@"0", nil];
    
    for (int c = 0; c < [rawMaterialDetails count]; c++) {
        NSArray *material = [rawMaterialDetails objectAtIndex:c];
        if ([[material objectAtIndex:0] isEqualToString:[NSString stringWithFormat:@"%@",[wareJSON_issue objectForKey:@"productName"]]]) {
    ...
    wareReceiptMaterialQuantity = 0;
    wareReceiptMaterialIssueCost = 0.0f;
    
    for (int i = 0; i < [rawMaterialDetails count]; i++) {
        NSArray *material = [rawMaterialDetails objectAtIndex:i];
        wareReceiptMaterialQuantity = wareReceiptMaterialQuantity + [[material objectAtIndex:7] intValue];
        wareReceiptMaterialIssueCost = wareReceiptMaterialIssueCost + ([[material objectAtIndex:2] floatValue] * [[material objectAtIndex:7] intValue]);
    }
    
    totalQuantity.text = [NSString stringWithFormat:@"%d",wareReceiptMaterialQuantity];
    totalCost.text = [NSString stringWithFormat:@"%.2f",wareReceiptMaterialIssueCost];
...
/**
 ...
 @abstract       call when the view is loaded
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    wareScrollValueIssueStatus = NO;
    wareTolocationScrollValueStatus_ = NO;
    ...
    location.placeholder = @"  To Location";
    ...
    fromLocation = [[UITextField alloc] init];
    fromLocation.borderStyle = UITextBorderStyleRoundedRect;
    fromLocation.textColor = [UIColor blackColor];
    fromLocation.font = [UIFont systemFontOfSize:18.0];
    fromLocation.backgroundColor = [UIColor whiteColor];
    fromLocation.clearButtonMode = UITextFieldViewModeWhileEditing;
    fromLocation.backgroundColor = [UIColor whiteColor];
    fromLocation.autocorrectionType = UITextAutocorrectionTypeNo;
    fromLocation.layer.borderColor = [UIColor whiteColor].CGColor;
    fromLocation.backgroundColor = [UIColor whiteColor];
    fromLocation.delegate = self;
    fromLocation.placeholder = @"   From Location";
    [fromLocation addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
    fromLocation.text = presentLocation;
    fromLocation.userInteractionEnabled = FALSE;
    ...
    poReference = [[UITextField alloc] init];
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        fromLocation.font = [UIFont boldSystemFontOfSize:20];
        fromLocation.frame = CGRectMake(400.0, 10.0, 360, 50);
        ...
        //  deliveredBy.font = [UIFont boldSystemFontOfSize:30];
        ...
        //
        //        poReference.font = [UIFont boldSystemFontOfSize:30];
        ...
        label5.frame = CGRectMake(381, 270, 110, 55);
        ...
        label9.frame = CGRectMake(496, 270, 110, 55);
        label10.font = [UIFont boldSystemFontOfSize:20];
        label10.frame = CGRectMake(614.0, 270, 110, 55);
        ...
        normalstockTable.frame = CGRectMake(0.0, 40.0, self.view.frame.size.width, 350.0);
    ...
    [createReceiptView addSubview:fromLocation];
    ...
    warehStartPoint = 1000;
    
    [self getAllReceiptIDS:warehStartPoint];
...
- (void)viewDidLoad {
    ...
    NSArray *segmentLabels = [NSArray arrayWithObjects:@"New Issue",@"View Issue", nil];
...
/**
 ...
 @function       callRawMaterials
 @abstract       call the service method (getRawMaterial) that gives the list of raw materils
 @param          searchString (NSString *)
...
-(void) callRawMaterials:(NSString *)searchString {
    ...
    @try {
        ...
        if (status) {
            ...
            NSArray *temp = [wareJSON_issue objectForKey:@"skuIds"];
            
            rawMaterials = [[NSMutableArray alloc] init];
            NSDictionary *skuidDic = [[NSDictionary alloc] init];
            
            for (int i=0; i<[temp count]; i++) {
                
                skuidDic = [temp objectAtIndex:i];
                
                [skuArrayList addObject:[skuidDic valueForKey:@"skuId"]];
...
/**
 ...
 @function       callReceiptIDS
 @abstract       call the service method (getStockIssueIds) that gives the list of stock issue ids...
 @param          searchString (NSString *)
...
-(void)callReceiptIDS:(NSString *)searchString {
    ...
    NSArray *keys = [NSArray arrayWithObjects:@"requestHeader",@"goods_issue_ref_num", nil];
    NSArray *objects = [NSArray arrayWithObjects:dictionary_,ReceiptID.text, nil];
    
    NSDictionary *reqDic = [NSDictionary dictionaryWithObjects:objects forKeys:keys];
    ...
    WHStockIssueServices_getStockIssueIds *create_receipt = [[WHStockIssueServices_getStockIssueIds alloc] init];
    create_receipt.searchCriteria = createReceiptJsonString;
    @try {
        WHStockIssueServicesSoapBindingResponse *response = [materialBinding getStockIssueIdsUsingParameters:create_receipt];
        ...
        for (id bodyPart in responseBodyParts) {
            ...
            if ([bodyPart isKindOfClass:[WHStockIssueServices_getStockIssueIdsResponse class]]) {
                
                WHStockIssueServices_getStockIssueIdsResponse *body = (WHStockIssueServices_getStockIssueIdsResponse *)bodyPart;
        ...
        if (status) {
            NSArray *temp =  [JSON objectForKey:@"issueIds"];
...
/**
 ...
 @abstract       delegate method  that is invoked whenever a textfield is changed
...
- (void)textFieldDidChange:(UITextField *)textField {
    if (textField == searchItem) {
        if ([textField.text length] >= 3) {
            ...
            for (NSString *product in skuArrayList)
            {
                NSComparisonResult result = [product compare:textField.text options:(NSCaseInsensitiveSearch|NSDiacriticInsensitiveSearch) range:NSMakeRange(0, [textField.text length])];
...
-(void)getListOfLocations:(id)sender {
    ...
    if (sender == selectLocation) {
        ...
        wareTolocationStartIndex = 0;
        
        [self getLocations:wareTolocationStartIndex];
...
-(void)getLocations:(int)startIndex {
    ...
    NSArray *loyaltyObjects = [NSArray arrayWithObjects:@"0",dictionary, nil];
    ...
    @try {
        ...
        if ([[responseHeader valueForKey:@"responseCode"] isEqualToString:@"0"] && [[responseHeader valueForKey:@"responseMessage"] isEqualToString:@"Location Details"]) {
            ...
            for (int i=0; i < [locations count]; i++) {
                ...
                NSDictionary *location1 = [locations objectAtIndex:i];
            ...
            wareTolocationScrollValueStatus_ = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == normalstockTable) {
        ...
        //        NSError *err;
        //        NSDictionary *procurmentDetails = [NSJSONSerialization JSONObjectWithData: [[NSString stringWithFormat:@"%@",[procuremnetReceiptDetails objectAtIndex:indexPath.row]] dataUsingEncoding:NSUTF8StringEncoding]
        //                                                                          options: NSJSONReadingMutableContainers
        //                                                                            error: &err];
        ...
        receiptID.text = [receiptsIdsArr objectAtIndex:indexPath.row];
        ...
        if ([[receiptStatusArr objectAtIndex:indexPath.row] isEqualToString:@"false"]) {
            ...
            statusView.frame = CGRectMake(700.0, 35.0, 50.0, 50.0);
        }
        ...
        total.frame = CGRectMake(500.0, 60.0, 200.0, 55.0);
        ...
        hlcell.textLabel.text = [rawMaterials objectAtIndex:indexPath.row];
        hlcell.textLabel.textColor = [UIColor blackColor];
...
/**
 ...
 @function       delRow
 @abstract       deletes the selected row of the table
 @param          sender (id)
...
-(void)changeQuantity:(UIButton *)sender {
    ...
    wareReceiptRejectMaterialTagId = sender.tag;
...
/**
 ...
 @function       qtyOkButtonPressed
 @abstract       quantity is changed
 @param          sender (id)
...
- (IBAction)qtyOkButtonPressed:(id)sender {
    ...
    NSArray *temp = [rawMaterialDetails objectAtIndex:wareReceieptMaterialTagid];
    ...
    if([value length] == 0 || !isNumber){
        ...
        [rawMaterialDetails replaceObjectAtIndex:wareReceiptRejectMaterialTagId withObject:finalArray];
...
/**
 ...
 @function       changeRejectQuantity
 @abstract       change quantity view
 @param          sender (UIButton *)
...
/**
 ...
 @function       qtyCancelButtonPressed
 @abstract        quantity view is cancelled
 @param          sender (id)
...
/**
 ...
 @function       rejectQtyCancelButtonPressed
 @abstract       reject quantity view is cancelled
 @param          sender (id)
...
/**
 ...
 @function       rejectQtyOkButtonPressed
 @abstract       reject quantity is entered
 @param          sender (id)
...
- (IBAction)rejectQtyOkButtonPressed:(id)sender {
    ...
    NSArray *temp = [rawMaterialDetails objectAtIndex:wareReceiptRejectMaterialTagId];
...
/**
 ...
 @function       tableView
 @abstract       delegate method called when a row of a table view is selected
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == skListTable) {
        ...
        [self callRawMaterialDetails:[rawMaterials objectAtIndex:indexPath.row]];
        ...
        OpenWareHouseIssue *viewReceipt = [[OpenWareHouseIssue alloc] initWithReceiptID:receiptID];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == normalstockTable) {
        ...
        if ((indexPath.section == lastSectionIndex) && (indexPath.row == lastRowIndex)) {
            ...
            if (!wareScrollValueIssueStatus) {
                wareStartIndexInt = wareStartIndexInt + [receiptsIdsArr count];
                [self getAllReceiptIDS:wareStartIndexInt];
                [normalstockTable reloadData];
            }
            else {
                
                dataStatus = [[[UILabel alloc] init] autorelease];
                dataStatus.text = @"No More Issue ID To Load";
...
/**
 ...
 @function       getAllReceiptIDS
 @abstract       calls the service method(getStockIssues) that gives all the receipt details based on the startIndex
...
-(void) getAllReceiptIDS:(int)startPoint{
    ...
    [receiptDetails1 setObject:[NSString stringWithFormat:@"%d",wareStartIndexInt] forKey:@"startIndex"];
    ...
    WHStockIssueServices_getStockIssues *stock_issues = [[WHStockIssueServices_getStockIssues alloc] init];
    stock_issues.searchCriteria = createReceiptJsonString;
    
    NSDictionary *json1;
    NSError *e;
    @try {
        WHStockIssueServicesSoapBindingResponse *response = [materialBinding getStockIssuesUsingParameters:stock_issues];
        NSArray *responseBodyParts1 = response.bodyParts;
        
        for (id bodyPart in responseBodyParts1) {
            
            if ([bodyPart isKindOfClass:[WHStockIssueServices_getStockIssuesResponse class]]) {
                
                
                [HUD setHidden:YES];
                WHStockIssueServices_getStockIssuesResponse *body = (WHStockIssueServices_getStockIssuesResponse *)bodyPart;
                printf("\nresponse=%s",[body.return_ UTF8String]);
        ...
        if (status) {
            ...
            if ([[responseDic valueForKey:@"responseCode"] isEqualToString:@"0"] && [[responseDic valueForKey:@"responseMessage"]isEqualToString:@"Getting ReciptIssue Details Successfully"]) {
                
                NSArray *arr = [json1 objectForKey:@"issueIds"];
                
                if ([arr count] == 0) {
                    wareScrollValueIssueStatus = YES;
                }
                ...
                for (int i=0; i<[arr count]; i++) {
                    ...
                    [receiptsIdsArr addObject:[receiptDtlsDic valueForKey:@"goods_issue_ref_num"]];
...
/**
 ...
 @abstract       called when the submit button is clicked
 @param          ...
...
-(void)submitButtonPressed {
    ...
    if ([rawMaterialDetails count] == 0){
        ...
        [receiptDetails1 setObject:presentLocation forKey:@"shipped_from"];
        [receiptDetails1 setObject:location.text forKey:@"issue_to"];
        ...
        [receiptDetails1 setObject:totalCost.text forKey:@"issue_total"];
        [receiptDetails1 setObject:totalQuantity.text forKey:@"issue_total_qty"];
        ...
        //          NSArray *keys = [NSArray arrayWithObjects:@"userName",@"accessKey",@"applicationName",@"dateAndTime",@"correlationId",@"userId",@"emailId",@"customerId",@"skuId",@"shipped_from",@"delivered_by",@"deliveryDate", nil];
        ...
        for (int i=0; i < [itemcode count]; i++) {
            ...
            [temp setObject:[rejected objectAtIndex:i] forKey:@"max_quantity"];
        ...
        //        NSArray *objects = [NSArray arrayWithObjects:@"sonali.maharana@technolabssoftware",@"CID8995418",@"Store Mobile APP",[NSString stringWithFormat:@"%@ %@",[[str[0] componentsSeparatedByString:@","] objectAtIndex:0],str[1]],@"-",@"CID8995418",@"sonali.maharana@technolabssoftware.com",@"CID8995418",rawMaterial, nil];
        ...
        WHStockIssueServices_createStockIssue *create_receipt = [[WHStockIssueServices_createStockIssue alloc] init];
        create_receipt.stockIssueDetails = createReceiptJsonString;
        
        WHStockIssueServicesSoapBindingResponse *response = [materialBinding createStockIssueUsingParameters:create_receipt];
        NSArray *responseBodyParts1 = response.bodyParts;
        
        for (id bodyPart in responseBodyParts1) {
            
            if ([bodyPart isKindOfClass:[WHStockIssueServices_createStockIssueResponse class]]) {
                
                
                [HUD_ setHidden:YES];
                WHStockIssueServices_createStockIssueResponse *body = (WHStockIssueServices_createStockIssueResponse *)bodyPart;
                printf("\nresponse=%s",[body.return_ UTF8String]);
                ...
                if (json1!=NULL) {
                    ...
                    wareJSON_issue = [json1 objectForKey:@"responseHeader"] ;
                    if ([[wareJSON_issue objectForKey:@"responseMessage"] isEqualToString:@"Success"]) {
                        ...
                        NSString *receiptID = [json1 objectForKey:@"issueid"];
                        warehReceipt = [receiptID copy];
                        UIAlertView *successAlertView  = [[UIAlertView alloc] init];
                        [successAlertView setDelegate:self];
                        [successAlertView setTitle:@"Material Transfer Issue Submitted Successfully"];
...
/**
 ...
 @abstract       called when the save button is pressed
 @param
            ...
            if ([bodyPart isKindOfClass:[WHStockIssueServices_createStockIssueResponse class]]) {
                ...
                if (json1!=NULL) {
                    ...
                    if ([[wareJSON_issue objectForKey:@"responseMessage"] isEqualToString:@"Success"]) {
                        ...
                        UIAlertView *successAlertView  = [[UIAlertView alloc] init];
                        [successAlertView setDelegate:self];
                        [successAlertView setTitle:@"Material Transfer Issue Saved Successfully"];
                        [successAlertView setMessage:[NSString stringWithFormat:@"%@%@",@"Receipt ID : ",receiptID]];
                        [successAlertView addButtonWithTitle:@"OPEN RECEIPT"];
                        [successAlertView addButtonWithTitle:@"NEW RECEIPT"];
...
/**
 ...
 @function       alertView
 @abstract       delegate method that is called when an alert is shown
 @param          alertView (UIAlertView *) , buttonIndex (NSInteger)
 
 */

- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    AudioServicesPlaySystemSound (soundFileObject);
    if ([alertView.title isEqualToString:@"Material Transfer Issue Submitted Successfully"]) {
        if (buttonIndex == 0) {
            self.navigationItem.backBarButtonItem.tintColor = [UIColor blackColor];
            self.navigationItem.backBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStyleBordered target:nil action:nil] autorelease];
            
            OpenWareHouseIssue *viewReceipt = [[OpenWareHouseIssue alloc] initWithReceiptID:warehReceipt];
            [self.navigationController pushViewController:viewReceipt animated:YES];
        }
        else{
    ...
    else if ([alertView.title isEqualToString:@"Material Transfer Issue Saved Successfully"]){
        if (buttonIndex == 0) {
            self.navigationItem.backBarButtonItem.tintColor = [UIColor blackColor];
            self.navigationItem.backBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStyleBordered target:nil action:nil] autorelease];
